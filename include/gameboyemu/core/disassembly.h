...
static const std::array<std::string, 256> InstructionNames = {
    "NOP",        "LD BC,NN",   "LD (BC),A",  "INC BC",     "INC B",
    "DEC B",      "LD B,N",     "RLCA",       "LD (NN),SP", "ADD HL,BC",
    "LD A,(BC)",  "DEC BC",     "INC C",      "DEC C",      "LD C,N",
    "RRCA",

    "STOP",       "LD (DE),NN", "LD DE,A",    "INC DE",     "INC D",
    "DEC D",      "LD D,N",     "RLA",        "JR S8",      "ADD HL,DE",
    "LD A,(DE)",  "DEC DE",     "INC E",      "DEC E",      "LD E,N",
    "RRA",

    "JR NZ,S8",   "LD HL,NN",   "LD (HL+),A", "INC HL",     "INC H",
    "DEC H",      "LD H,N",     "DAA",        "JR Z,S8",    "ADD HL,HL",
    "LD A,(HL+)", "DEC HL",     "INC L",      "DEC L",      "LD L,N",
    "CPL",

    "JR NC,S8",   "LD SP,NN",   "LD (HL-),A", "INC SP",     "INC (HL)",
    "DEC (HL)",   "LD (HL),N",  "SCF",        "JR C,S8",    "ADD HL,SP",
    "LD A,(HL-)", "DEC SP",     "INC A",      "DEC A",      "LD A,N",
    "CCF",

    "LD B,B",     "LD B,C",     "LD B,D",     "LD B,E",     "LD B,H",
    "LD B,L",     "LD B,(HL)",  "LD B,A",     "LD C,B",     "LD C,C",
    "LD C,D",     "LD C,E",     "LD C,H",     "LD C,L",     "LD C,(HL)",
    "LD C,A",
    ...
    "LD H,B",     "LD H,C",     "LD H,D",     "LD H,E",     "LD H,H",
    "LD H,L",     "LD H,(HL)",  "LD H,A",     "LD L,B",     "LD L,C",
    "LD L,D",     "LD L,E",     "LD L,H",     "LD L,L",     "LD L,(HL)",
    "LD L,A",

    "LD (HL),B",  "LD (HL),C",  "LD (HL),D",  "LD (HL),E",  "LD (HL),H",
    "LD (HL),L",  "HALT",       "LD (HL),A",  "LD A,B",     "LD A,C",
    "LD A,D",     "LD A,E",     "LD A,H",     "LD A,L",     "LD A,(HL)",
    "LD A,A",

    "ADD A,B",    "ADD A,C",    "ADD A,D",    "ADD A,E",    "ADD A,H",
    "ADD A,L",    "ADD A,(HL)", "ADD A,A",    "ADC A,B",    "ADC A,C",
    "ADC A,D",    "ADC A,E",    "ADC A,H",    "ADC A,L",    "ADC A,(HL)",
    "ADC A,A",

    "SUB A,B",    "SUB A,C",    "SUB A,D",    "SUB A,E",    "SUB A,H",
    "SUB A,L",    "SUB A,(HL)", "SUB A,A",    "SBC A,B",    "SBC A,C",
    "SBC A,D",    "SBC A,E",    "SBC A,H",    "SBC A,L",    "SBC A,(HL)",
    "SBC A,A",
    ...
    "OR A,B",     "OR A,C",     "OR A,D",     "OR A,E",     "OR A,H",
    "OR A,L",     "OR A,(HL)",  "OR A,A",     "CP A,B",     "CP A,C",
    "CP A,D",     "CP A,E",     "CP A,H",     "CP A,L",     "CP A,(HL)",
    "CP A,A",

    "RET NZ",     "POP BC",     "JP NZ,NN",   "JP NN",      "CALL NZ,NN",
    "PUSH BC",    "ADD A,N",    "RST 0",      "RET Z",      "RET",
    "JP Z,NN",    "CALL Z,NN",  "CALL NN",    "ADC A,N",    "RST 1",

    "RET NC",     "POP DE",     "JP NC,NN",   "CALL NC,NN", "PUSH DE",
    "SUB A,N",    "RST 2",      "RET C",      "RETI",       "JP C,NN",
    "CALL C,NN",  "SBC A,N",    "RST 3",

    "LD (N),A",   "POP HL",     "LD (C),A",   "PUSH HL",    "AND A,N",
    "RST 4",      "ADD SP,N",   "JP HL",      "LD (NN),A",  "XOR A,N",
    "RST 5",

    "LD A,(N)",   "POP AF",     "LD A,(C)",   "DI",         "PUSH AF",
    "OR A,N",     "RST 6",      "LD HL,SP+N", "LD SP,HL",   "LD A,(NN)",
    "EI",         "CP A,N",     "RST 7",
};
